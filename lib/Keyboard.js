...
class VirtualKeyboard extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.options.updateOnChange === true) {
      this.props.options.change = (event, keyboard, el) => {
        this.handleChange(event, keyboard.preview.value);
        if (this.props.debug) {
  ...
  handleChange = (event, input) => {
    ...
    this.props.onChange(input, event);
  }
  ...
  render() {
    ...
    else if(this.props.options.type === 'custom')
      element = (
        <div>
          <span ref={node => this.keyboardRef = node} value = { this.state.value } onChange = { this.handleChange } {...other}/>
          <b className='caret' style={{left: `${this.props.cursorStyle}vh`}} >B</b>
        </div>
      )
    else
      element = (
